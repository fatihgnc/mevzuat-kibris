 ...
 * lose its meaning. Auth is only needed for /takip, /hesap and /auth/*. /karar/* is
 * in the matcher too now, but only for the rate limiter above — it returns before
 * ever reaching the Supabase session logic below.
 */
...
export const config = {
  matcher: ['/takip/:path*', '/hesap/:path*', '/auth/:path*', '/karar/:path*'],
};
